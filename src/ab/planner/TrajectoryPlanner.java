/*****************************************************************************
 ** ANGRYBIRDS AI AGENT FRAMEWORK Copyright (c) 2013, XiaoYu (Gary) Ge, Stephen
 * Gould,Jochen Renz Sahan Abeyasinghe, Jim Keys, Kar-Wai Lim, Zain Mubashir,
...
public class TrajectoryPlanner
{
	...
	protected static double X_OFFSET = 0.5;
	protected static double Y_OFFSET = 0.65;
	protected static double BOUND = 0.1;
	protected static double STRETCH = 0.4;
	protected static int X_MAX = 640;

	// 10, 15, 20, 25, 30, 35, 40, 45, 50, 55, 60, 65, 70
	protected static double _launchAngle[] = { 0.13, 0.215, 0.296, 0.381,
												0.476, 0.567, 0.657, 0.741,
												0.832, 0.924, 1.014, 1.106,
												1.197 };
	protected static double _changeAngle[] = { 0.052, 0.057, 0.063, 0.066,
												0.056, 0.054, 0.050, 0.053,
												0.042, 0.038, 0.034, 0.029,
												0.025 };
	protected static double _launchVelocity[] = { 2.9, 2.88, 2.866, 2.838,
													2.810, 2.800, 2.790, 2.773,
													2.763, 2.745, 2.74, 2.735,
													2.73 };

	// small modification to the scale and angle
	protected double _scaleFactor = 1.005;

	// conversion between the trajectory time and actual time in milliseconds
	protected double _timeUnit = 815;

	// boolean flag on set trajectory
	protected boolean _trajSet = false;

	// parameters of the set trajectory
	protected Point _release;
	protected double _theta, _velocity, _ux, _uy, _a, _b;

	// the trajectory points
	protected ArrayList<Point> _trajectory;
			...
			Rectangle slingshot, Point releasePoint)
	{
		List<Point> trajectory = predictTrajectory(slingshot, releasePoint,
				canvas.getWidth(null) - slingshot.x);
	...
	protected List<Point> predictTrajectory(Rectangle slingshot,
			Point launchPoint, int X_MAX)
	{
	...
	// angle required
	protected double actualToLaunch(double theta)
	{
	...
	// trajectory
	protected double launchToActual(double theta)
	{
		for (int i = 1; i < _launchAngle.length; i++)
		{
			if (theta > _launchAngle[i - 1] && theta < _launchAngle[i])
	...
	protected void adjustScale(double v, double theta)
	{
		int i = 0;
	...
	protected double getVelocity(double theta)
	{
	...
	protected double getSceneScale(Rectangle sling)
	{
